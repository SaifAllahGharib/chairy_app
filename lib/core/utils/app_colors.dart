import 'package:flutter/material.dart';

abstract class AppColors {
  static const Color primaryColor = Color(0xffE58411);
  static const Color secColor = Color(0xffFCEFE0);
  static const Color white = Colors.white;
  static const Color black = Colors.black;
  static const Color lightBlack = Color(0xff262626);
  static const Color semiDarkBlack = Color(0xff2F2F2F);
  static const Color midnightBlue = Color(0xff0D1B39);
  static const Color darkGray = Color(0xff1E1E1E);
  static const Color gray = Color(0xffD9D9D9);
  static const Color lightGray = Color(0xffF0F0F0);
  static const Color bgNavigationBar = Color(0xffF8F8F8);
  static const Color bgDarkNavigationBar = Color(0xff2A2A2A);
  static const Color lightColor = Color(0xffFFF9F1);
  static const Color darkColor = Color(0xff221506);
  static const Color medGray = Color(0xff8D8D8D);
  static const Color darkBlue = Color(0xff0E192B);
  static const Color lightGreen = Color(0xffDAFFDB);
  static const Color green = Color(0xff1EC815);
  static const Color circleColor = Color(0xff0F2764);
  static const Color bgRegister = Color(0xffFBFBFB);
  static const Color red = Color(0xffED1C24);
}
